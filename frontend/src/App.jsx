...
export default function App() {
  ...
  useEffect(() => {
    if (currentPage === 'exp') {
      fetch('http://localhost:8000/api/tech-experience')
        .then(res => res.json())
        .then(data => { setTechExperience(Array.isArray(data) ? data : []); setTechExperienceLoaded(true); })
  ...
  if (currentPage === 'exp') {
    ...
    const techCategories = [...new Set(techExperience.map(t => t.category))];
    const filteredExperience = techExperience.filter(item =>
            ...
            <main className="main-content">
              <div className="exp-page">
                <div className="exp-header">
                  ...
                  <span className="exp-count">{filteredExperience.length} of {techExperience.length}</span>
                </div>
